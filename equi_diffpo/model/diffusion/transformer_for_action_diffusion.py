...
class TransformerForActionDiffusion(ModuleAttrMixin):
    ...
    def configure_optimizers(self, 
            ...
            weight_decay: float=1e-3,   # 权重衰减（L2 正则化系数），抑制模型复杂度，防止过拟合
            betas: Tuple[float, float]=(0.9,0.95)):# 一阶和二阶动量更新的动量系数
        # 模型参数分组，每组有独立的权重衰减或学习率
        # 按weight、bias开头和结尾分了4组
        optim_groups = self.get_optim_groups(weight_decay=weight_decay)
        # 改进了 Adam 优化器中权重衰减（L2 正则化）的实现方式，使其更加简洁

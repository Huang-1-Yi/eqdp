# 版本：1.3
# 说明1：删除无用注释和输出
# 说明2：使用len直接读取，而不是基于时间戳长度
# 说明3：添加保存数据集时的内存使用监控，创建+保存前-中-后的4次内存使用监控
# 说明4：添加self.image_pred_accumulators，保存预测图像
# 版本：1.2
# 说明1：参考钟浩的代码，修改图像保存功能的实现方式，增加了对齐深度图像的处理，增加了对齐点云的处理，增加了对齐时间戳的处理，增加了对齐图像的处理，增加了对齐点云的处理
# 说明2：增加self.is_saving，实现采集过程的阻塞保存问题
# 版本：1.1
# 说明：增加_is_recording，二重保护，确定是否开始录制，确定在退出等情况下会坚持保存，避免出错
# 版本：1.0
# 说明：增加图像保存功能，增加了对齐时间戳的处理，增加了对齐深度图像的处理，增加了对齐点云的处理
...
class RealEnvFranka:
    def __init__(self, 
            ...
            # action
            max_pos_speed       = 0.25,                 # 最大位置速度0.25
            max_rot_speed       = 1.0,                  # 最大旋转速度0.6
            # robot
        ...
        if self.enable_sam2:  # 如果启用sam2
            self.image_pred_accumulators = dict()                   # 每个摄像头一个累加器
    ...
    def get_obs(self) -> dict:
        ...
        for camera_idx, value in self.last_realsense_data.items():  # 遍历每个相机的数据
            if self.obs_accumulator is not None:
                ...
                if self.enable_sam2:
                    self.image_pred_accumulators[camera_idx].put(
                        masks=value['color_pred'],  # shape (N, 480, 640, 3)
                        timestamps=value['timestamp_pred']  # shape (N,)
                    )
            ...
            if self.enable_pointcloud and 'point_cloud' in value:
                ...
                if 'point_cloud' in value:
                    ...
                    print(f"Warning: 'point_cloud' not found in value for camera {camera_idx}.")
            ...
            # 对齐rgb数据
            if self.enable_sam2:
                camera_obs[f'camera_{camera_idx}'] = value['color_pred'][aligned_idxs] # 将颜色数据映射到相机观察字典中，(n_obs_steps, H, W, 3)
            else:
    ...
    def exec_actions(self, 
        ...
        for i in range(len(new_actions)):           # 遍历新动作
            ...
            self.robot.schedule_gripper_command(
                state=gripper_action,
        ...
        if return_flag[0]:
            ...
            # print("return_flag[0]==",return_flag[0])
            print("env.move_to_start")
        # else:
        #     print("return_flag[0]",return_flag[0])
    ...
    def start_episode(self, start_time=None):
        ...
        for cam_idx in range(n_cameras):
            ...
            if self.enable_sam2:
                ###################新增rgb图像mask累加器###################
                self.image_pred_accumulators[cam_idx] = TimestampMaskAccumulator(
                    start_time=start_time,
                    dt=1 / self.frequency,
    ...
    def end_episode(self):
        ...
        if self.obs_accumulator is not None:                        # 如果观察累加器不为空
            ...
            if self.enable_sam2:
                image_timestamps = {}
                for cam_idx, accum in self.image_pred_accumulators.items():
                    print(f"camera_{cam_idx} pred images: {len(accum.masks)}")
                    print(f"camera_{cam_idx} pred timestamps: {len(accum.timestamps)}")
                    # 将每个相机的时间戳存储在字典中，key 为相机的索引，value 为对应的时间戳数组
            ...
            if self.verbose:# 使用实际理论长度校验
                ...
                if self.enable_sam2:
                    image_pred_acc_actual_lengths = [acc.actual_length for acc in self.image_pred_accumulators if acc is not None]
                    print(f"  - Image Pred Actual Lengths: {image_pred_acc_actual_lengths}") 
                    image_pred_min = min(image_pred_acc_actual_lengths)
                    n_steps = min(obs_len, action_len, stage_len, image_min, image_pred_min)
                else:
            ...
            data_lengths = {
                ...
                **{f"camera_{cam_idx}": len(accum.images) for cam_idx, accum in self.image_accumulators.items()},
                # **{f"camera_{cam_idx}_pred": len(accum.masks) for cam_idx, accum in self.image_pred_accumulators.items()}
            }
            ...
            if self.enable_sam2:
                for cam_idx, accum in self.image_pred_accumulators.items():
                    try:
                        episode[f'camera_{cam_idx}_pred'] = self.image_pred_accumulators[cam_idx].masks[:n_steps]
                    except Exception as e:
                        print(f"Camera {cam_idx} 预测数据异常: {e}")
                        episode[f'camera_{cam_idx}_pred'] = np.zeros((n_steps, *self.obs_image_resolution), dtype=np.uint8)
            ...
            for accum in self.image_accumulators.values():
                accum.clear()
            if self.enable_sam2:
                for accum in self.image_pred_accumulators.values():
                    accum.clear()
            # 清空相机数据累加器
            if self.enable_depth:

...
from eqdp.model.vision.multi_image_obs_encoder  import MultiImageObsEncoder
...
class DiffusionUnetImagePolicy(BaseImagePolicy):
    ...
    def compute_loss(self, batch):
        ...
        # # umi 的改进：在训练过程中，向扩散模型的输入数据添加额外的随机噪声 用于​缓解扩散模型训练中的暴露偏差（Exposure Bias）问题
        # # input perturbation by adding additonal noise to alleviate exposure bias
        # # reference: https://github.com/forever208/DDPM-IP
        # noise = noise + self.input_pertub * torch.randn(trajectory.shape, device=trajectory.device) # noise_new
